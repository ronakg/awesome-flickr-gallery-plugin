 ...
 */
add_filter( 'upgrader_source_selection', 'afg_rename_update_zip', 1, 3);

/**
 * Removes the suffix -<tagname> when updating from GitHub zip files
 * 
 ...
 * @return string
 */
function afg_rename_update_zip( $source, $remote_source, $thiz )
{
    if(  strpos( $source, PLUGIN_SLUG) === false )
        return $source;

    $path_parts = pathinfo( $source );
    $newsource = trailingslashit( $path_parts['dirname'] ) . trailingslashit( PLUGIN_SLUG );
